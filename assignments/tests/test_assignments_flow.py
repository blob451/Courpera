...
from assignments.models import Assignment, AssignmentType, QuizQuestion, QuizAnswerChoice, Attempt


def make_teacher(username="teacher"):
    u = User.objects.create_user(username=username, password="Strong#Passw0rd", email=f"{username}@ex.com")
    prof = u.profile
...
def make_student(username="student"):
    return User.objects.create_user(username=username, password="Strong#Passw0rd", email=f"{username}@ex.com")


def make_ready_quiz(course, title="Quiz 1"):
    a = Assignment.objects.create(course=course, type=AssignmentType.QUIZ, title=title, attempts_allowed=2)
...
def test_paper_submission_validates_filetype(client):
    ...
    course = Course.objects.create(owner=teacher, title="C1")
    Enrolment.objects.create(course=course, student=student)
    a = Assignment.objects.create(course=course, type=AssignmentType.PAPER, title="Paper 1", is_published=True, available_from=timezone.now())
    ...
    resp = client.post(reverse("assignments:submit", args=[a.id]), {"submission_file": bad}, follow=True)
...
def test_attempts_cannot_be_lowered_below_used(client):
    ...
    Attempt.objects.create(assignment=a, student=student, attempt_no=1, submitted_at=timezone.now(), score=0)
    client.force_login(teacher)
