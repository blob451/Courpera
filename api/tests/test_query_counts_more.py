...
class TestMoreQueryCounts(TestCase):
    def setUp(self):
        ...
        self.teacher.profile.role = "teacher"; self.teacher.profile.save(update_fields=["role"])
        self.student = User.objects.create_user(username="sqc2", password="pw")
        self.student.profile.role = "student"; self.student.profile.save(update_fields=["role"])
        ...
        for i in range(3):
            ...
            Material.objects.create(course=self.course, uploaded_by=self.teacher, title=f"M{i}", file=f)

    def test_materials_list_query_budget_student(self):
        c = Client(); assert c.login(username="sqc2", password="pw")
