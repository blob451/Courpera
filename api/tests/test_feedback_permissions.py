...
def test_feedback_requires_enrolment_and_allows_update_by_owner():
    ...
    client = Client(); assert client.login(username='sfeed', password='pw')
    ...
    fb_id = r.json().get('id')
    # Update own feedback
    r2 = client.patch(f'/api/v1/feedback/{fb_id}/', {
        'rating': 3
    }, content_type='application/json')
    ...
    s2.profile.role = 'student'; s2.profile.save(update_fields=['role'])
    client.logout(); assert client.login(username='sfeed2', password='pw')
