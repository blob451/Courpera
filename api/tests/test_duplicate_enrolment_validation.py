...
def test_duplicate_enrolment_returns_400_not_500():
    ...
    c = Client(); assert c.login(username="dup_t", password="pw")
    r = c.post("/api/v1/courses/", {"title": "D", "description": ""}, content_type="application/json")
    assert r.status_code in (200, 201)
    course_id = r.json()["id"]

    cs = Client(); assert cs.login(username="dup_s", password="pw")
