...
def test_materials_serializer_includes_expected_fields():
    ...
    client = Client(); assert client.login(username="smat", password="pw")
    r = client.get(f"/api/v1/materials/?course={c.id}")
    assert r.status_code == 200
    data = r.json()
    row = data["results"][0]
    assert {"id", "course", "title", "size_bytes", "mime", "created_at", "file_url"} <= set(row.keys())
...
def test_course_detail_forbidden_when_not_owner_or_enrolled():
    ...
    t.profile.role = "teacher"; t.profile.save(update_fields=["role"])
    c = Client(); assert c.login(username="ownr", password="pw")
    # Create a course via API to get id easily
    r_create = c.post("/api/v1/courses/", {"title": "X", "description": ""}, content_type="application/json")
    assert r_create.status_code in (200, 201)
    course_id = r_create.json().get("id") or 1

    # Anonymous forbidden
    anon = Client(); r = anon.get(f"/api/v1/courses/{course_id}/")
    assert r.status_code == 403

    # Different logged-in user also forbidden
    u2 = User.objects.create_user(username="other", password="pw")
    c2 = Client(); assert c2.login(username="other", password="pw")
...
def test_course_detail_query_budget_owner():
    ...
    c = Client(); assert c.login(username="qdet", password="pw")
