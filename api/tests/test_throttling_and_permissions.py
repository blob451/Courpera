...
def test_anon_throttle_limits_requests():
    ...
    assert codes[3] in (429, 200)  # allow some leeway if cache isn’t warmed; but should typically be 429
...
def test_teacher_only_course_create():
    ...
    ct = Client(); assert ct.login(username='t', password='pw')
    cs = Client(); assert cs.login(username='s', password='pw')
    r_ok = ct.post('/api/v1/courses/', {'title': 'A', 'description': ''}, content_type='application/json')
