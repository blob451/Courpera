...
def test_post_without_csrf_is_rejected_on_html_view():
    ...
    c = Client(enforce_csrf_checks=True); assert c.login(username="csrfu", password="pw")
...
def test_student_and_anon_cannot_access_teacher_search_html():
    ...
    cs = Client(); assert cs.login(username="std", password="pw")
    r = cs.get("/accounts/search/?q=x")
    assert r.status_code == 403
    # anon also forbidden
    anon = Client(); r2 = anon.get("/accounts/search/?q=x")
