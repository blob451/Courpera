...
def ensure_student_number(sender, instance: UserProfile, **kwargs):  # noqa: D401
    ...
    """
    if getattr(instance, "role", None) == Role.STUDENT and not getattr(instance, "student_number", ""):  # type: ignore[attr-defined]
...
def ensure_instructor_id(sender, instance: UserProfile, **kwargs):  # noqa: D401
    ...
    if getattr(instance, "role", None) == Role.TEACHER and not getattr(instance, "instructor_id", None):  # type: ignore[attr-defined]
        if getattr(instance, "user_id", None):
